...
import { Camera, Mic, Zap, Leaf, Shield, Info, Loader2, BarChart3, Utensils, Settings, QrCode, Plus, CreditCard } from "lucide-react";
...
export default function FoodLensDemo({ 
  ...
  async function analyzeFrame() {
    ...
    try {
      ...
      // Add to macro tracking and database
      if (user?.id) {
        console.log('💾 Saving food entry to database for user:', user.id);
        try {
      ...
      console.error('Analysis error (using mock fallback):', error);
      // Fallback to mock data so users can still save entries
      try {
        await new Promise(resolve => setTimeout(resolve, 800));
        const mockResult = {
          name: 'Sample Food',
          nutrition: { calories: 250, protein_g: 15, fat_g: 8, carbs_g: 30 },
          pros: ['Good protein content'],
          cons: ['Could be lower in fat'],
          confidence: 0.85,
          health_score: 7
        };
        const calculatedScore = mockResult.health_score || computeHealthScore(mockResult.nutrition);
        const stamped = { ...mockResult, score: calculatedScore, lastUpdated: new Date().toLocaleTimeString() };
        setResult(stamped);
        if (user?.id) {
          console.log('💾 Saving MOCK food entry to database for user:', user.id);
          try {
            await addFoodEntry(stamped, user.id);
